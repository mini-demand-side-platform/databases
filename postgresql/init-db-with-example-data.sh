...
EOSQL
# pg_dump -U dsp -d olap -t ctr -t feature -t feature_store -t top_10_features | gzip > olap_database.sql.gz
# gunzip < /olap_database.sql.gz |psql --username "$POSTGRES_USER" --dbname olap 
